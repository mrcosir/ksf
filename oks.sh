...
==================================================================
		...
		妖火大帝免流--WEB流量控制--云免服务器一键搭建				
			Powered by losir.tk 2015-2016					
				All Rights Reserved							
...
====================================================================';
	...
	webfile='yhdd-web.zip';
	lnmpfile=yhdd-lnmp.tar.gz;
	uploadfile=yhdd-openvpn.tar.gz;
	return 1
}
...
function readytoinstall() {
	...
	killall mproxy-yhdd >/dev/null 2>&1
	rm -rf /etc/openvpn/*
	rm -rf /root/*
	rm -rf /home/*
	sleep 2 
...
rm -rf /etc/openvpn/yhdd.sh
echo "载入妖火大帝流量控制openvnp配置"
clear
...
then
	...
	verb 3
	#losir.tk" >/etc/openvpn/server.conf
	cd /etc/openvpn/
	rm -rf /easy-rsa/
...
#if [[ $mysqlversion == '5.4' ]]
#then
#	curl -O https://github.com/mu228/ksf/raw/master/phpMyAdmin-4.0.10.15-all-languages.tar.gz
#	tar -zxvf phpMyAdmin-4.0.10.15-all-languages.tar.gz -C /home/wwwroot/default/
#	rm -f phpMyAdmin-4.0.10.15-all-languages.tar.gz
#else
#	curl -O https://github.com/mu228/ksf/raw/master/phpMyAdmin-4.6.2-all-languages.tar.gz
...
lnmp
 echo "安装完成！"
 echo "感谢使用妖火大帝一键极速LNMP - Centos7版"
 return 1
}
function webml(){
clear
...
echo "* * * * * /root/mproxy-yhdd -l 137 -d">>/var/spool/cron/root
systemctl restart crond.service    
...
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd-quanguo1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd1-quanguo-1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd2-quanguo-1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd-zj1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd-gd1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-yd-gx-quanguo1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-lt-quanguo1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-lt-quanguo11.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-lt-guangdong1.ovpn
echo '########免流代码########
resolv-retry infinite
nobind
persist-key
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">111dx1.ovpn
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-dx12.ovpn
...
echo "# 妖火大帝云免配置 电信常规-测试免广东-康师傅自用广东电信
# 本文件由系统自动生成
# 类型：2-常规类型
client
dev tun
...
########免流代码########
http-proxy $IP 137
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-ydkbd.ovpn
echo '
...
########免流代码########
http-proxy $IP $mpport
http-proxy-option EXT1 yhdd 127.0.0.1:$vpnport">http-ydyd.ovpn
echo '
http-proxy-option EXT1 "POST http://rd.go.10086.cn/ HTTP/1.1"
http-proxy-option EXT1 "Host: rd.go.10086.cn" 
########免流代码########
...
cd /home
rm -rf root/kangapk 
mv /home/vpn.apk YunAPP.apk
echo "制作本地云端APP完成"
echo
echo "进行打包文件..."
sleep 2
cd /home/
tar -zcvf ${uploadfile} ./{YunAPP.apk,yd-kbaidu.ovpn,yd-1.ovpn,yd-2.ovpn,yd-3.ovpn,yd-4.ovpn,yd-gd.ovpn,yd-gx.ovpn,yd-zj.ovpn,lt-1.ovpn,lt-2.ovpn,lt-gd.ovpn,dx-1.ovpn,dx-2.ovpn,dx-3.ovpn,yd-5.ovpn,lt.ovpn,ca.crt,ta.key,info.txt} >/dev/null 2>&1
echo "上传文件..."
sleep 2
...
echo
echo "欢迎使用妖火大帝一键脚本进行搭建！"
echo -e '\033[33mby：妖火大帝\033[0m'
authentication
InputIPAddress
clear
clear
echo "下面要设置一些信息"
echo "如果不懂/新手，记住新手脚本理念：遇到不会的就回车！"
echo
echo "回车开始吧~"
read
clear
...
clear
echo "说明："
echo "两台服务器必须都已安装妖火大帝WEB流控并成功连接！"
...
clear
echo "请根据实际情况填写，让妖火大帝脚本知道你想怎么更新"
echo
...
rm -rf /home/ca.crt
rm -rf /home/ta.eky
rm -rf /home/yhdd-openvpn.tar.gz
cp /etc/openvpn/easy-rsa/keys/ca.crt /home/ >/dev/null 2>&1
cp /etc/openvpn/easy-rsa/ta.key /home/ >/dev/null 2>&1
mpport=8080
rm -rf /bin/vpn
...
echo "*--欢迎使用妖火大帝Web流控--*"
echo
echo "请输入妖火大帝Web流控搭建授权码:"
read card
echo "正在获取数据..."
echo ">>>>>>>>>>>>>>>>>"
echo
echo -e '恭喜您！ [\033[32m  授权成功  \033[0m]';
echo "此授权码已绑定您的服务器IP，授权码永久使用！";
